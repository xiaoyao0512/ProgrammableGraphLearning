...
int main(int argc, char const *argv[])
{
    ...
    FILE* taskGraphOut = fopen("taskGraph.ps","wb");
    ...
    if (!dataArrowsEnabled)
    {
        ...
        while((loopCount < MAX_GRAPH_NODE_SIZE) && (currentTask = tg->readContechTask()))
        {
            ...
            cout <<loopCount<<"::"<< "UID: " << currentTask->getTaskId().toString() <<" # "<<currentTask->getType()<<" # ";
		...
		vector<TaskId> y=currentTask->getSuccessorTasks();
		vector<TaskId>::iterator it;	
		for(it=y.begin();it!=y.end();it++)
		printf("%lu ",uint64_t(*it));
		printf("$");
		y=currentTask->getPredecessorTasks();
		for(it=y.begin();it!=y.end();it++)
		printf("%lu ",uint64_t(*it));
		printf("\n");//
    ...
    gvLayout(gvc, g, "dot");
    gvRender(gvc, g, "ps", taskGraphOut);
