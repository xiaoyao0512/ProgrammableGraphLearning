...
//  the more general matches.
    ...
    bool Contech::runOnModule(Module &M)
    {
        ...
        unsigned long int instr_count = 0;
        int length = 0;
        char* buffer = NULL;
        doInitialization(M);
        ...
        for (Module::iterator F = M.begin(), FE = M.end(); F != FE; ++F) {
            ...
            std::cout << "current function name: " << name << std::endl;
            ...
            for (Function::iterator B = F->begin(), BE = F->end(); B != BE; ++B) {
                BasicBlock &pB = *B;
                std::cout << "#BasicBlock: " << bb_count << std::endl;
                internalRunOnBasicBlock(pB, M, bb_count, ContechMarkFrontend, fmn);
                bb_count++;
                //std::cout << "#BB: " << bb_count << std::endl;
            }
        ...
        std::cout << "END" << std::endl;
        //std::cout << "tot instr: " << instr_count << std::endl;
...
cleanup:
        ...
        //std::cout << "tot instr: " << ContechStateFilename.c_str() << std::endl;
        if (buffer == NULL)
        {
    ...
    bool Contech::internalRunOnBasicBlock(BasicBlock &B,  Module &M, int bbid, const bool markOnly, const char* fnName)
    {
        ...
        for (BasicBlock::iterator I = B.begin(), E = B.end(); I != E; ++I)
        {
            ...
            //fprintf(fp, (const char *)*I);
            
            errs() << *I << "\n";
            //   compare with getFirstInsertionPt
            Instruction *ins = convertIterToInst(I);
            const char *name = ins->getOpcodeName();
            //std::cout << "INSTR" << name << endl;
            if (/*PHINode *pn = */dyn_cast<PHINode>(&*I))
            {
                getNextI = true;
                numIROps --;
                //std::cout << "PHINODE" << endl;
                continue;
            }
            else if (/*LandingPadInst *lpi = */dyn_cast<LandingPadInst>(&*I))
            {
        ...
        //std::cout << "PUPUPU" << fileName << std::endl;
