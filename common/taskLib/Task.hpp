...
public:
    void print_alist(FILE* fp){
	//fprintf(fp,"Print_alist() called\n");
	vector<Action> y=a;
	vector<Action>::iterator it=y.begin();
	for(;it!=y.end();it++){
		//fprintf(fp,"+\n");
		it->ac_str(fp);
		//fprintf(fp," %lu\n",it->data);
    ...
    void print_splist(FILE* fp){
	vector<TaskId> y=s;
	vector<TaskId>::iterator it=y.begin();
	fprintf(fp,"S:");
	for(;it!=y.end();it++){
		fprintf(fp,"%s ",it->toString().c_str());}
	fprintf(fp,"\nP:");
	y=p;
	it=y.begin();
	for(;it!=y.end();it++){
