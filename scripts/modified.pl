...
while (<IN>) {
	...
	if ($BB) {
		...
		push @{$BBs{$index}}, $_ unless (/#BasicBlock|END|current|SKIP|is struct/);
		#print "$_\n" if (/br/);
		#print;
		#print "\n";
	}
...
my $flag = 0;
my $counterIter = 0;
foreach my $nodeList (@sortedArr1) {
	my $value1 = ${$nodeList}[0];
	my $value2 = ${$nodeList}[1];
	my $value3 = ${$nodeList}[2];
