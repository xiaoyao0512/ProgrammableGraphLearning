...
system("clang++ -emit-llvm -S -c $file -o llvm.ll -std=c++11 -fno-use-cxa-atexit");
...
my @timing;
my @printf;
#system("lli llvm_instrumented.ll @arguments");
#if (defined $ARGV[1]) {
...
#modified for c++ version
#system("nice -n -19 ./contech_wrapper_par++.py $file -lm -std=c++11 > log");
system("./contech_wrapper_par++.py $file -lm -std=c++11 > log");
print("LALALALALALAL\n");
...
system("../middle/middle /tmp/contech_fe Sta > temp");
open(IN, "temp");
my @weights = (1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096);
open(OUT, ">$file.ll");
...
foreach my $idx (0..($numOfEdges - 1)) {
	...
	"target=\"${$sortedArr1[$idx]}[1].0\" weight=\"${$sortedArr1[$idx]}[2]\" label=\"${$sortedArr1[$idx]}[2]\"/>\n";
}
print OUT2 "</edges>\n";
print OUT2 "</graph>\n";
print OUT2 "</gexf>\n";
