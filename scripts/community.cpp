...
Community::Community(char * filename, char * filename_w, int type, int nbp, double minm, int N, double lambda1, double lambda2) {
  ...
  //cout << "lalala" << lambda2 << endl;
 
  n2c.resize(size);
  in.resize(size);
  tot.resize(size);
  sizeForComm.resize(size);
...
Community::Community(Graph gc, int nbp, double minm, int N, double lambda1, double lambda2) {
  g = gc;
  size = g.nb_nodes;
  //cout << "WA" << endl;
  neigh_weight.resize(size,-1);
...
Community::modularity() {
  ...
  for (int i = 0; i < size - 1; i++) {
  	if (tot[i] > 0) { 
  		R1 +=  fabs(in[i] - in[(i + 1)]) / (m2);
  	}
  }
  q -= lambda1*R1;
  double R2 = 0.0;
  if (nc > N) {
  	R2 = 1.0 * lambda2 * (nc - N) / (nc);
  }
  //cout << "R22 = " << R2 << endl;
  q -= R2;
  return q;
...
//      int node = node_tmp;
      int node = node_tmp;//random_order[node_tmp];
//      int node = random_order[node_tmp];
      ...
      //cout << "node " << node_comm << endl;
      double best_nblinks  = 0.;
      double best_increase = 0.;
      //cout << "neigh_last " << neigh_last << endl; 
      for (unsigned int i=0 ; i<neigh_last ; i++) {
      	//neigh_comm(node);
      	//cout << "node = " << node << ", comm = " << neigh_pos[i] << endl;
      ...
      //if (best_comm != node_comm && n2c[node_comm]) 
    }
